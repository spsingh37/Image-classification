...
def svm_loss(x, y):
    ...
    scores = x
    num_train = x.shape[0]
    correct_class_scores = scores[torch.arange(0,x.shape[0]),y].view(-1,1)
    loss_mask = scores - correct_class_scores + 1
    margin = torch.max(torch.zeros_like(scores), loss_mask)
    margin[torch.arange(0,num_train),y] = 0
    loss = torch.sum(margin)/num_train 

    dx = torch.zeros_like(margin)
    dx[margin > 0] = 1
    num_cnt = torch.sum(dx, dim = 1)
    dx[torch.arange(x.shape[0]), y] = -num_cnt
    dx = dx/x.shape[0]
    #####################################################
    #                  END OF YOUR CODE                 #
    #####################################################
...
def softmax_loss(x, y):
    ...
    scores = scores - max_scores.view(-1,1)
    
    correct_class_scores = scores[torch.arange(0,x.shape[0]),y].view(-1,1)
    exp_correct = torch.exp(correct_class_scores)
    exp_sum = torch.sum(torch.exp(scores),axis=1).view(-1,1)
    loss = -torch.log(exp_correct/exp_sum)
    loss = torch.sum(loss)/num_train

    dx = torch.exp(scores)/ (exp_sum.view(-1, 1))
    dx[torch.arange(num_train), y] = (-1 * (exp_sum.view(-1,1) - exp_correct) / exp_sum.view(-1,1)).view(-1)
    dx = dx/num_train
    #####################################################
    #                  END OF YOUR CODE                 #
    #####################################################
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        exp_scores_sum = torch.sum(exp_scores, axis =1)
        correct_exp_score = exp_scores[torch.arange(0,N),y]
        norm_scores = correct_exp_score/exp_scores_sum
        loss = -torch.sum(torch.log(norm_scores))/N + self.reg*torch.sum(W1*W1) + self.reg*torch.sum(W2*W2)

        c_fct = exp_scores/ (exp_scores_sum.view(-1, 1))
        c_fct[torch.arange(N), y] = (-1 * (exp_scores_sum - correct_exp_score) / exp_scores_sum)
        grads['W2'] = torch.mm(hidden.t(),c_fct) / N + 2 * self.reg * W2
        grads['b2'] = torch.sum(c_fct, dim=0) / N

        upstr_grad = torch.mm(c_fct, W2.t())
        upstr_grad[hidden==0] = 0
        grads['W1'] = torch.mm(X.t(),upstr_grad)/N + 2 * self.reg * W1 
        grads['b1'] = torch.sum(upstr_grad, dim=0)/N
        ###################################################################
        #                     END OF YOUR CODE                            #
        ###################################################################
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(len(hidden_dims)):
          self.params[f"W{i+1}"] = weight_scale*torch.randn(prev_dim, hidden_dims[i], dtype = dtype, device = device)
          self.params[f"b{i+1}"] = torch.zeros(hidden_dims[i], dtype= dtype, device = device)
          prev_dim = hidden_dims[i]
                  
        self.params[f"W{i+2}"] = weight_scale*torch.randn(prev_dim, num_classes, dtype = dtype, device = device)
        self.params[f"b{i+2}"] = torch.zeros(num_classes, dtype= dtype, device = device)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
          ...
          score_out = torch.mm(score_in,W) + b
          score_out[score_out<=0] = 0
          if self.use_dropout:
            score_out, cache_dict[f"{i+1}"] = Dropout.forward(score_out, self.dropout_param)
          score_in = score_out
          hid_lay_scores[f"{i+1}"] = score_out
        W, b = self.params[f"W{i+2}"], self.params[f"b{i+2}"]
        scores = torch.mm(score_in,W) + b
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
        ...
        for i in range(self.num_layers):
          reg_loss += 0.5*self.reg*torch.sum(self.params[f"W{i+1}"]*self.params[f"W{i+1}"]) 
        
        loss = -torch.sum(torch.log(norm_scores))/N + reg_loss

        upstr_grad = exp_scores/ (exp_scores_sum.view(-1, 1))
        upstr_grad[torch.arange(N), y] = (-1 * (exp_scores_sum - correct_exp_score) / exp_scores_sum)
        
        for i in reversed(range(self.num_layers-1)):
          grads[f"W{i+2}"] = torch.mm(hid_lay_scores[f"{i+1}"].t(),upstr_grad) / N + self.reg * self.params[f"W{i+2}"]
          grads[f"b{i+2}"] = torch.sum(upstr_grad, dim=0) / N

          upstr_grad = torch.mm(upstr_grad, self.params[f"W{i+2}"].t())
          if self.use_dropout:
            upstr_grad = Dropout.backward(upstr_grad, cache_dict[f"{i+1}"])
          upstr_grad[hid_lay_scores[f"{i+1}"]==0] = 0
        grads[f"W{i+1}"] = torch.mm(X.t(),upstr_grad) / N + self.reg * self.params[f"W{i+1}"]
        grads[f"b{i+1}"] = torch.sum(upstr_grad, dim=0) / N
        ###########################################################
        #                   END OF YOUR CODE                      #
        ###########################################################
...
def create_solver_instance(data_dict, dtype, device, **kwargs):
    ...
    solver = None
    # Replace "pass" statement with your code
    solver = Solver(model,data = data_dict, device = device)
    ##############################################################
    #                    END OF YOUR CODE                        #
    ##############################################################
    return solver
...
def rmsprop(w, dw, config=None):
    ...
    next_w = w - config['learning_rate']*dw/(torch.sqrt(grad_squared) + epsilon)
...
def adam(w, dw, config=None):
    ...
    next_w = w - config['learning_rate']*m_unbias/(v_unbias.sqrt() + epsilon)
    config['t'] = t
    config['m'] = m
    config['v'] = v
    #########################################################################
...
class Dropout(object):
    ...
    def backward(dout, cache):
        ...
        if mode == 'train':
            ...
            # Replace "pass" statement with your code
            p = dropout_param['p']
            mask_grad = torch.ones_like(dout)/(1-p)
            mask_grad[mask==0] = 0
            dx = mask_grad*dout
            ###########################################################
            #                     END OF YOUR CODE                    #
