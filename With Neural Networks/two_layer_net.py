...
def nn_forward_backward(
    ...
    loss = -torch.sum(torch.log(norm_scores))/N + reg*torch.sum(W1*W1) + reg*torch.sum(W2*W2)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    ...
    grads['W2'] = torch.mm(h1.t(),c_fct) / N + 2 * reg * W2
    grads['b2'] = torch.sum(c_fct, dim=0) / N

    upstr_grad = torch.mm(c_fct, W2.t())
    upstr_grad[h1==0] = 0
    grads['W1'] = torch.mm(X.t(),upstr_grad)/N + 2 * reg * W1 
...
def nn_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [0.001, 0.01, 0.1, 1.0, 1.15 ]
    hidden_sizes = [16, 32, 128, 256]
    regularization_strengths = [0.00001, 0.001, 0.01]
    learning_rate_decays = [1.0, 0.95, 0.92, 0.90]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def find_best_net(
    ...
    #############################################################################
    # Replace "pass" statement with your code
    lrs, hidden_sizes, rgs, learning_rate_decays = get_param_set_fn()
      ...
      for hs in hidden_sizes:
        for rg in rgs:
          for lr_decay in learning_rate_decays:
            ...
            current_stat_best_val_acc = torch.max(torch.tensor(stats["val_acc_history"]))
            
            if current_stat_best_val_acc > best_val_acc:
              best_val_acc = current_stat_best_val_acc
              best_net = net
              best_stat = stats              
