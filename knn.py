...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_test):
      flat_test_img = x_test[i].contiguous().view(-1)      
      for j in range(num_train):
        flat_train_img = x_train[j].contiguous().view(-1)
        dists[j][i] = (flat_test_img - flat_train_img).pow(2).sum().sqrt()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x_test_reshaped = x_test.clone().reshape(x_test.shape[0],-1)
    
    for i in range(num_train):
      flat_train_img = x_train[i].contiguous().view(-1)   
      dists[i,:] = torch.sqrt(torch.sum((x_test_reshaped - flat_train_img)**2, axis=1))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x_train_reshaped = x_train.contiguous().view((num_train, -1))
    print(x_train_reshaped.shape)
    x_test_reshaped = x_test.contiguous().view(num_test, -1)
    dists = torch.sqrt(torch.sum(x_test_reshaped**2, dim=1) + torch.sum(x_train_reshaped**2, dim=1).view(-1, 1) - 2 * torch.mm(x_train_reshaped, x_test_reshaped.t()))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(dists.shape[1]):
      distances, indices = torch.topk(dists[:,i], k = k, largest = False, sorted = True)
      nearest_labels = y_train[indices]
      sorted_labels = nearest_labels.unique(sorted=True)
      label_counts = nearest_labels.bincount()
      label_counts = label_counts[label_counts.nonzero()].view(-1)
      rev_label_counts = torch.flip(label_counts, dims=(0,))
      rev_sorted_labels = torch.flip(sorted_labels, dims=(0,))
      max_count = rev_label_counts[0]
      max_count_label = rev_sorted_labels[0]
      for j in range(len(rev_sorted_labels)):
        if max_count<=rev_label_counts[j] and rev_sorted_labels[j]<=max_count_label:
          max_count_label = rev_sorted_labels[j]
        
      y_pred[i] = max_count_label
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    for k in k_choices:
      ...
      for i in range(num_folds):
        x_train_cat = torch.cat([x_train[:i*x_train_folds[i].shape[0]], x_train[(i+1)*x_train_folds[i].shape[0]:]])
        y_train_cat = torch.cat([y_train[:i*y_train_folds[i].shape[0]], y_train[(i+1)*y_train_folds[i].shape[0]:]])
        x_val_set = x_train[i*x_train_folds[i].shape[0]:i*x_train_folds[i].shape[0]+x_train_folds[i].shape[0]]
        y_val_set = y_train[i*y_train_folds[i].shape[0]:i*y_train_folds[i].shape[0]+y_train_folds[i].shape[0]]
        classifier = KnnClassifier(x_train_cat, y_train_cat)
        acc.append(classifier.check_accuracy(x_val_set, y_val_set, k))
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k,v in k_to_accuracies.items():
      mean_acc = sum(v)/ float(len(v))
      if mean_acc>best_acc:
        best_k = k
        best_acc = mean_acc
      elif mean_acc==best_acc and k<best_k:
        best_k = k 
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
